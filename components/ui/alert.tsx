  ...
  "relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground backdrop-blur-md",
  {
    variants: {
      variant: {
        ...
        // Added for Claim Verifier Verdicts:
        success:
          "border-primary/50 text-primary dark:border-primary [&>svg]:text-primary bg-primary/10",
        warning:
          "border-pink/50 text-pink dark:border-pink [&>svg]:text-pink bg-pink/10",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("text-sm font-body opacity-90 [&_p]:leading-relaxed", className)}
    {...props}
  />
));
